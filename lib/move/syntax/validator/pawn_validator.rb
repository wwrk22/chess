...
class PawnValidator
  ...
  def validate(move_str, color)
    ...
    return move_str.include?('x') ?
      validate_capture_move(move_str, color) :
      validate_non_capture_move(move_str, color)
  end

  # Validate a move that is a capture. Raise ColorUnknownError if color is
  # unknown.
  def validate_capture_move(move_str, color)
  ...
  # is unknown.
  def validate_non_capture_move(move_str, color)
