require 'support/matchers/chess_piece'
require './lib/board/board'
require './lib/move/computer/pawn_start_computer'
require './lib/piece/chess_piece'
require './lib/piece/piece_specs'
require './lib/move/move'


RSpec.configure do |cfg|
  cfg.include PieceSpecs
end

RSpec.describe PawnStartComputer do
  describe '#compute_capture_start' do
    context "when pawn is white" do
      subject(:computer) { described_class.new }

      it "returns a square whose rank is one less than the target" do
        target_square = { file: 'a', rank: 3 }
        expected_start = { file: 'b', rank: 2 }
        move = instance_double(Move)
        allow(move).to receive(:target).and_return(target_square)

        computed_start = computer.compute_capture_start(white, expected_start[:file], target_square)
        expect(computed_start).to eq(expected_start)
      end
    end

    context "when pawn is black" do
      subject(:computer) { described_class.new }

      it "returns a square whose rank is one more than the target" do
        target_square = { file: 'a', rank: 6 }
        expected_start = { file: 'b', rank: 7 }
        move = instance_double(Move)
        allow(move).to receive(:target).and_return(target_square)

        computed_start = computer.compute_capture_start(black, expected_start[:file], target_square)
        expect(computed_start).to eq(expected_start)
      end
    end # context "when pawn is black"
  end # describe '#compute_capture'


  describe '#compute_capture' do
    subject(:computer) { described_class.new }

    it "returns a boolean to indicate whether the starting square could be determined" do
      move = instance_double(Move)
      board = instance_double(Board)
      white_pawn = instance_double(ChessPiece)
      expected_start = { file: 'b', rank: 2 }

      allow(white_pawn).to receive(:type).and_return(pawn)
      allow(white_pawn).to receive(:color).and_return(white)

      allow(move).to receive(:piece).and_return(white_pawn)
      allow(move).to receive(:start_coordinate).and_return('b')
      allow(move).to receive(:target).and_return({ file: 'a', rank: 3 })

      allow(board).to receive(:at).with(expected_start[:file], expected_start[:rank]).and_return(white_pawn)

      result = computer.compute_capture(move, board)
      expect(result).to eq(expected_start)
    end
  end # describe '#compute_capture'


  describe '#compute_move' do
    subject(:computer) { described_class.new }

    context "when the pawn is black" do
      let(:move) { instance_double(Move) }
      let(:board) { instance_double(Board) }
      let(:black_pawn) { ChessPiece.new(pawn, black) }

      before do
        allow(move).to receive(:piece).and_return(black_pawn)
        allow(move).to receive(:target).and_return({ file: target_file, rank: target_rank })
      end

      context "when the move is a single" do
        let(:target_file) { 'a' }
        let(:target_rank) { 6 }

        context "when the square above the target square is empty" do
          it "returns nil" do
            allow(board).to receive(:at).with(target_file, target_rank + 1).and_return nil

            result = computer.compute_move(move, board)
            expect(result).to be_nil
          end
        end

        context "when the square above the target square is not a black pawn" do
          it "returns nil" do
            black_rook = ChessPiece.new(rook, black)
            allow(board).to receive(:at).with(target_file, target_rank + 1).and_return(black_rook, black_rook)

            result = computer.compute_move(move, board)
            expect(result).to be_nil
          end
        end

        context "when the square above the target square has a black pawn" do
          it "returns the square" do
            allow(board).to receive(:at).with(target_file, target_rank + 1).and_return black_pawn

            result = computer.compute_move(move, board)
            expected = { file: target_file, rank: target_rank + 1 }
            expect(result).to eq(expected)
          end
        end
      end # context "when the move is a single"

      context "when the move is a double" do
        let(:target_file) { 'b' }
        let(:target_rank) { 5 }

        context "when the square two above the target square is empty" do
          it "returns nil" do
            allow(board).to receive(:at).with(target_file, target_rank + 1).and_return nil
            allow(board).to receive(:at).with(target_file, target_rank + 2).and_return nil

            result = computer.compute_move(move, board)
            expect(result).to be_nil
          end
        end

        context "when the square two above the target square is not a black pawn" do
          it "returns nil" do
            black_knight = ChessPiece.new(knight, black)
            allow(board).to receive(:at).with(target_file, target_rank + 1).and_return nil, nil
            allow(board).to receive(:at).with(target_file, target_rank + 2).and_return(black_knight)

            result = computer.compute_move(move, board)
            expect(result).to be_nil
          end
        end

        context "when the square two above the target square is a black pawn" do
          it "returns the square" do
            allow(board).to receive(:at).with(target_file, target_rank + 1).and_return nil, nil
            allow(board).to receive(:at).with(target_file, target_rank + 2).and_return(black_pawn)

            result = computer.compute_move(move, board)
            expected = { file: target_file, rank: target_rank + 2 }
            expect(result).to eq(expected)
          end
        end
      end # context "when the move is a double"
    end # context "when the pawn is black"

    context "when the pawn is white" do
      let(:move) { instance_double(Move) }
      let(:board) { instance_double(Board) }
      let(:white_pawn) { ChessPiece.new(pawn, white) }

      before do
        allow(move).to receive(:piece).and_return(white_pawn)
        allow(move).to receive(:target).and_return({ file: target_file, rank: target_rank })
      end

      context "when the move is a single" do
        let(:target_file) { 'a' }
        let(:target_rank) { 3 }

        context "when the square below the target square is empty" do
          it "returns nil" do
            allow(board).to receive(:at).with(target_file, target_rank - 1).and_return nil

            result = computer.compute_move(move, board)
            expect(result).to be_nil
          end
        end

        context "when the square below the target square is not a white pawn" do
          it "returns nil" do
            white_rook = ChessPiece.new(rook, white)
            allow(board).to receive(:at).with(target_file, target_rank - 1).and_return(white_rook, white_rook)

            result = computer.compute_move(move, board)
            expect(result).to be_nil
          end
        end

        context "when the square below the target square has a white pawn" do
          it "returns the square" do
            allow(board).to receive(:at).with(target_file, target_rank - 1).and_return white_pawn

            result = computer.compute_move(move, board)
            expected = { file: target_file, rank: target_rank - 1 }
            expect(result).to eq(expected)
          end
        end
      end # context "when the move is a single"

      context "when the move is a double" do
        let(:target_file) { 'b' }
        let(:target_rank) { 4 }

        context "when the square two below the target square is empty" do
          it "returns nil" do
            allow(board).to receive(:at).with(target_file, target_rank - 1).and_return nil
            allow(board).to receive(:at).with(target_file, target_rank - 2).and_return nil

            result = computer.compute_move(move, board)
            expect(result).to be_nil
          end
        end

        context "when the square two below the target square is not a white pawn" do
          it "returns nil" do
            white_knight = ChessPiece.new(knight, white)
            allow(board).to receive(:at).with(target_file, target_rank - 1).and_return nil, nil
            allow(board).to receive(:at).with(target_file, target_rank - 2).and_return(white_knight)

            result = computer.compute_move(move, board)
            expect(result).to be_nil
          end
        end

        context "when the square two below the target square is a white pawn" do
          it "returns the square" do
            allow(board).to receive(:at).with(target_file, target_rank - 1).and_return nil, nil
            allow(board).to receive(:at).with(target_file, target_rank - 2).and_return(white_pawn)

            result = computer.compute_move(move, board)
            expected = { file: target_file, rank: target_rank - 2 }
            expect(result).to eq(expected)
          end
        end
      end # context "when the move is a double"
    end # context "when the pawn is black"
  end # describe '#compute_move'
end
