...
RSpec.describe RookArbiter do
  ...
  describe '#check_target' do
    ...
    context "when the target square on the board is not empty" do
      ...
      end # context "when the chess piece on the square matches neither type nor color"
  ...
  describe '#check_start' do
    let!(:file) { 'a' }
    let!(:rank) { 4 }
    let!(:start) { { file: file, rank: rank } }
    let!(:test_type) { Piece::RO }
    let!(:test_color) { Piece::WH }
    let!(:wh_rook) { { type: test_type, color: test_color } }
    let!(:board) { instance_double(Board) }

    context "when there is only one possible start square" do
      context "when the one square is empty" do
        before do
          allow(board).to receive(:at).with(file, rank).and_return(nil)
        end

        it "returns nil" do
          expect(arbiter.check_start(board, start, wh_rook)).to be_nil
        end
      end # context "when the one square is empty"

      context "when the one square is not empty" do
        context "when the chess piece on the square matches type and color" do
          before do
            allow(board).to receive(:at).with(file, rank).and_return({ type: test_type, color: test_color })
          end

          it "returns the square" do
            expect(arbiter.check_start(board, start, wh_rook)).to eq(start)
          end
        end # context "when the chess piece on the square matches type and color"
        ...
        end # context "when the chess piece on the square matches the type only"
        ...
        end # context "when the chess piece on the square matches the color only"
      ...
      end # context "when the one square is not empty"
    end # context "when there is only one possibl start square"
  end
end
